...
//
// Usage:
//   pnpm label-das                 # label unlabelled DAs interactively
...
//
// Writes DaGroundTruth rows (labeller + timestamp + source). Runs against the
// DB in DATABASE_URL — point it at the docker test DB or staging, never prod
// unless you mean it.
...
function parseArgs(argv: string[]): Args {
  const args: Args = { labeller: process.env.USER ?? "founder", limit: 25, importThumbs: false };
...
async function labelInteractive(args: Args): Promise<void> {
  ...
  if (queue.length === 0) {
    console.log(`Nothing left to label for "${args.labeller}" — every DA already has a label from them.`);
    return;
  }

  console.log(
    `Labelling as "${args.labeller}". ${hits.length} rule-hits + ${misses.length} rule-misses queued.`,
  );
