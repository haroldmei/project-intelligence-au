...
1. Read `docs/03-system-design.md` for data models and relationships.
2. Read `docs/02-system-requirements.md` for data requirements.
3. Read `docs/01-market-analysis.md` for domain context (needed for realistic seed data).
4. Find and read the current schema file:
   - Prisma: `prisma/schema.prisma`
   - SQL: any `*.sql` migration files
   - Other ORMs: the relevant model/schema files
5. Check `docs/04-dev-plan.md` for any pending schema-related tasks.

## Phase 2 — Migration Audit

1. List all existing migrations: `ls prisma/migrations/ 2>/dev/null` or equivalent.
2. Check migration status: `npx prisma migrate status` (or equivalent).
