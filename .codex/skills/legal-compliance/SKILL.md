...
4. Scan the codebase for:
   - Cookie usage (text search for `cookie`, `localStorage`, `sessionStorage`)
   - Analytics/tracking scripts (text search for `analytics`, `gtag`, `posthog`, `mixpanel`)
...
5. **Data Sharing** — List all third-party processors:
   ...
   - Stripe — payments
   - SendGrid — email
   - AI providers (for example OpenAI or Anthropic) — AI processing
   - Twilio — SMS
